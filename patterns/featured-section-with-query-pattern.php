<?php
/**
 * Title: Featured Section With query Pattern
 * Slug: patterns-store/featured-section-with-query-pattern
 * Categories: featured
 * Keywords: Featured Section
 */

?>
<!-- wp:group {"align":"full","style":{"spacing":{"padding":{"top":"80px","bottom":"80px"}}},"backgroundColor":"primary","layout":{"type":"constrained"}} -->
<div class="wp-block-group alignfull has-primary-background-color has-background" style="padding-top:80px;padding-bottom:80px">

<!-- wp:pattern {"slug":"patterns-store/featured-section-title"} /-->

<!-- wp:spacer {"height":"60px"} -->
<div style="height:60px" aria-hidden="true" class="wp-block-spacer"></div>
<!-- /wp:spacer -->

<!-- wp:pattern {"slug":"patterns-store/query-pattern-grid"} /-->

</div>
<!-- /wp:group -->
